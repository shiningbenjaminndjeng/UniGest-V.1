// src/pages/Messagerie.jsx — Messagerie privée en temps réel
import { useState, useEffect, useRef } from 'react';
import { useAuth } from '../contexts/AuthContext';
import { useSocket } from '../contexts/SocketContext';
import api from '../utils/api';
import toast from 'react-hot-toast';
import { Search, Send, MessageSquare, ArrowLeft, Circle } from 'lucide-react';
...
export default function Messagerie() {
  ...
  // Si un user_id est passé en param (depuis Talent), ouvrir directement
  useEffect(() => {
    const userId = searchParams.get('user');
    if (userId && !selectedUser) {
      api.get(`/messagerie/users/search?q= `).then(() => {});
  ...
  return (
    <div className="fade-in" style={{ height: 'calc(100vh - 100px)' }}>
      <div className="flex h-full rounded-2xl overflow-hidden border" style={{ borderColor: 'var(--c-border)' }}>
            ...
            {searchResults.length > 0 && (
              <div className="mt-2 rounded-xl border overflow-hidden" style={{ borderColor: 'var(--c-border)' }}>
                {searchResults.map(u => {
                  ...
                  return (
                    <button key={u.id} onClick={() => { openConversation(u); setSearchUser(''); setSearchResults([]); }}
                      className="flex items-center gap-2 w-full p-2.5 text-left transition-all hover:bg-opacity-100"
                      ...
                      <div>
                        <p className="text-xs font-semibold" style={{ color: 'var(--c-text)' }}>{u.prenom} {u.nom}</p>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="text-center p-6">
                <MessageSquare size={32} className="mx-auto mb-2" style={{ color: 'var(--c-text-dim)' }} />
          ...
          {!selectedUser ? (
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              <p className="text-sm text-center max-w-xs" style={{ color: 'var(--c-text-muted)' }}>
                Sélectionnez une conversation ou cherchez un utilisateur pour commencer à discuter
              </p>
            </div>
          ) : (
            <>
              {/* Header chat */}
              <div className="flex items-center gap-3 p-4 border-b" style={{ borderColor: 'var(--c-border)', background: 'var(--c-surface)' }}>
                <button onClick={() => setSelectedUser(null)} className="p-1.5 rounded-lg transition-all md:hidden"
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {loadingMsgs ? (
                  <div className="flex items-center justify-center py-10">
                    <span className="w-6 h-6 border-2 border-primary rounded-full animate-spin"
                  ...
                  messages.map((msg, idx) => {
                    ...
                    return (
                      <div key={msg.id}>
                        ...
                        <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} slide-up`}>
                          {!isMe && (
                    ...
                    <div className="msg-other py-2.5 px-4">
                      <div className="flex gap-1">
                        <span className="w-1.5 h-1.5 rounded-full animate-bounce"
                          style={{ background: 'var(--c-text-muted)', animationDelay: '0ms' }} />
                        <span className="w-1.5 h-1.5 rounded-full animate-bounce"
                          style={{ background: 'var(--c-text-muted)', animationDelay: '150ms' }} />
                        <span className="w-1.5 h-1.5 rounded-full animate-bounce"
                          style={{ background: 'var(--c-text-muted)', animationDelay: '300ms' }} />
                      </div>
                    </div>
                  </div>
                )}
              ...
              {/* Input message */}
              <div className="p-4 border-t" style={{ borderColor: 'var(--c-border)', background: 'var(--c-surface)' }}>
                ...
                <p className="text-xs mt-1.5" style={{ color: 'var(--c-text-dim)' }}>
